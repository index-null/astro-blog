---
# ---- REQUIRED ----
title: "Hono's Onion Model — How Middleware Composition Works"
...
[Hono](https://hono.dev) is a fast and lightweight web framework that supports multi-runtime. Its RPC-like API makes TypeScript fullstack communication more efficient with type hints. Hono's high performance largely comes from its simple `middleware compose` logic.
...
## Extra parameter: the `next`

When I first looked at Hono's `compose()` function signature, I was confused by the optional `next` parameter:
...
My initial assumption was that this `next` parameter was used to build middleware chains in a **hierarchical way** — first composing global middleware, then passing route-specific handlers as `next` to continue execution. This would allow Hono to reuse global middleware across different route matches.

**However, after diving deeper into the Hono source code, I realized this assumption was wrong.**

### The truth: `next` is not used in the core flow

Looking at the core request handling logic in `hono-base.ts` ([link](https://github.com/honojs/hono/blob/c78932d745cdf6284ae131a156479ac930da0262/src/hono-base.ts#L454)):
...
const context = await composed(c); // ← No `next` parameter passed!
```

**The core `compose()` call does NOT pass any `next` parameter.**

This means Hono does **not** use `next` for hierarchical middleware composition. Instead, all matched middleware and route handlers are composed into a single chain via `matchResult[0]`.

### So what is the `next` parameter actually for?

After more research, I found that the `next` parameter is primarily used for **sub-app mounting** (`app.route()`).
...
Hono needs a way to pass control from the parent app to the sub-app. This is where `next` comes in. The implementation can be found in `hono-base.ts` ([link](https://github.com/honojs/hono/blob/c78932d745cdf6284ae131a156479ac930da0262/src/hono-base.ts#L224)).
...
However, understanding the sub-app mounting mechanism entirely requires knowledge of Hono's routing and path matching system, which is quite interesting and deserves its own deep dive.

Don't worry if this seems confusing right now, I'll break it down step by step in upcoming posts, where cover Hono's routing mechanism and matching algorithm in depth.
