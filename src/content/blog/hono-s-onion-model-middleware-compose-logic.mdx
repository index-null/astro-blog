...
Hono is a fast and lightweight web framework for Node.js. It's RPC-like API makes TypeScript fullstack FE-BE communication more efficient with type hint. Hono's high performance largely comes from its simple `middleware compose` logic.

I dug into Hono's middleware compose logic and implemented a minimal `onion model` from scratch.

# Basic usage of Hono's Middle·ware
...
There is something maybe confusing:

- recursive function `dispatch`
- the `closure` variable `index`
- when composing, there is an extra `next` parameter passed in

TBH, the extra `next` is the most intriguing part. But before we delve too deeply into it, let's just try to build a simple middleware processing pipeline by plain `Promise` and a little recursive function.
...
Apparently, we all know the running order of these two middleware is `m1_timer -> m2_req_logger -> router`, and then the response will be processed by `m2_req_logger` and followed by `m1_timer` again.
...
If you are familiar with how the `Promise` works, it will be easy to understand that its subsequent middleware will be executed again — which means all the subsequent middlewares along with the router handler will be executed twice `cascadingly`.
...
A plain way to solve this problem is to use a tracking variable to keep the middleware index won't be called again. So we need a one-time variable to track it for each handling of incoming request. `Closure` variable is an ideal solution.
...
Obviously, the final `index` stops at the `router`, but when the `m2` was called the second time, the `i` is still `1` because that `dispatch(1)` was passed to it.

## To Be Continued

I will continue to explain why the real Hono `compose()` realization asks for an extra parameter `next` which we thought it's intriguing and not necessary.
